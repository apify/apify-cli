...
describe('apify actor generate-schema-types', () => {
	...
	describe('key-value store schema', () => {
		...
		it('should not emit duplicate interfaces for collections with colliding names', async () => {
			const outputDir = joinPath('kvs-output-collisions');
			...
			await setupActorConfig(joinPath(), {
				kvsSchemaRef: {
					...
					title: 'Colliding KVS',
					collections: {
						'my-store': { contentTypes: ['application/json'], key: 'A', jsonSchema },
						my_store: { contentTypes: ['application/json'], key: 'B', jsonSchema },
					},
				},
			});
			...
			const generatedFile = await readFile(joinPath('kvs-output-collisions', 'key-value-store.ts'), 'utf-8');
			expect(generatedFile).toContain('export interface MyStore ');
			expect(generatedFile).toContain('export interface MyStore2 ');
		});
