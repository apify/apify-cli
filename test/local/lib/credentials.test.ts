...
describe('credentials', () => {
	...
	describe('file backend', () => {
		beforeEach(() => {
			vitest.stubEnv('APIFY_DISABLE_KEYRING', '1');
			writeAuthFile(v2AuthFile() as Record<string, unknown>);
			writeFileSyncSpy.mockClear();
		});
	...
	describe('ensureSecretsKeyed()', () => {
		it('moves keyring entries off the fixed names onto the user ID', async () => {
			vitest.stubEnv('APIFY_DISABLE_KEYRING', '');
			writeAuthFile(v2AuthFile({}, { secretsBackend: 'keyring' }) as Record<string, unknown>);
		...
		it('moves top-level file secrets into the profile', async () => {
			vitest.stubEnv('APIFY_DISABLE_KEYRING', '1');
			writeAuthFile(
				v2AuthFile({}, { secretsBackend: 'file', token: 'tok', proxy: { password: 'pw' } }) as Record<string, unknown>,
			);
		...
		it('drops the legacy keyring entries even when APIFY_DISABLE_KEYRING=1 is set', async () => {
			vitest.stubEnv('APIFY_DISABLE_KEYRING', '1');
		...
		it('is a no-op on a file whose secrets are already keyed', async () => {
			vitest.stubEnv('APIFY_DISABLE_KEYRING', '1');
			writeAuthFile(v2AuthFile({ token: 'tok' }, { secretsBackend: 'file' }) as Record<string, unknown>);
			writeFileSyncSpy.mockClear();
		...
		it('is memoized within a process', async () => {
			vitest.stubEnv('APIFY_DISABLE_KEYRING', '1');
			writeAuthFile(v2AuthFile({}, { secretsBackend: 'file', token: 'tok' }) as Record<string, unknown>);
			await ensureSecretsKeyed();
			expect(readProfile().token).toBe('tok');

			writeAuthFile(v2AuthFile({}, { secretsBackend: 'file', token: 'tok2' }) as Record<string, unknown>);
			await ensureSecretsKeyed();
	...
	describe('getLocalUserInfo()', () => {
		it('on file backend, reads the token and proxy password from the profile', async () => {
			vitest.stubEnv('APIFY_DISABLE_KEYRING', '1');
			writeAuthFile(
				v2AuthFile({ token: 'tok', proxy: { password: 'pw' } }, { secretsBackend: 'file' }) as Record<string, unknown>,
			);
	...
	describe('getApifyClientOptions()', () => {
		...
		it('resolves the stored token when nothing overrides it', async () => {
			await setSecret(TEST_USER_ID, 'token', 'tok_stored');
